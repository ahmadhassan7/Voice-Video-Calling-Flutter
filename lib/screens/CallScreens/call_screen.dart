...
class _CallScreenState extends State<CallScreen> {
  ...
  Future<void> _initAgoraRtcEngine() async {
    ...
    if (widget.call.isAudioCall) {
      await AgoraRtcEngine.disableVideo();
    } else {
      await AgoraRtcEngine.enableVideo();
    }
    // await AgoraRtcEngine.enableAudio();
    // final RtmCallManager = RtmClient.getRtmCallManager();
  }
  ...
  List<Widget> _getRenderViews() {
    final List<AgoraRenderWidget> list = [
      ...
      // AgoraRenderWidget(1, local: true, preview: true),
    ];
      ...
      body: Center(
        child: Stack(
          children: <Widget>[
            _viewRows(),
            // _panel(),
            !widget.call.isAudioCall ?Container()
                :Container(
              height: Get.height,
              width: Get.width,
              child: Center(
                child: Text('Connected with ${widget.call.callerName}'),
              ),
            ),
            !widget.call.isAudioCall ? _toolbar()
            :Positioned(
              bottom: 100,
              left: (Get.width/3)-20,
              child: _toolbar(),
