...
class _AddParticipantsPageState extends State<AddParticipantsPage> {
  ...
  void initState() {
    ...
    if (widget.edit == true) {
      ...
      for (int i = 0; i < widget.users.length; i++) {
        participants.add(widget.users[i]);
  ...
  void decide() async {
    ...
    if (widget.edit == true) {
      ...
      // final updatedGroup = database.getGroupData(widget.group.id);
      Navigator.pop(context);
    } else {
      NewGroupPage.participants = participants;
      Navigator.pop(context);
      // Navigator.push(
      //   context,
      //   MaterialPageRoute(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          "Add  Participants",
          // style: TextStyle(fontFamily: 'Courgette', letterSpacing: 1.25, fontSize: 24),
        ),
      ),
      body: StreamBuilder(
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                return CheckboxListTile(
                  value: participants.contains(allUsersList[index]["uid"])
                      ? true
                      : false,
                  title: Text(
                      '${allUsersList[index]["name"]}'), //${users[index].lname}'),
                  // subtitle: Text(
                  //   '${users[index].email}',
